   ...
   private void Update()
{
    ...
    bool isAiming = weaponHandler != null && weaponHandler.IsAiming;
    ...
    if (!isAiming && _moveInput != Vector2.zero)
    {
    ...
    private void TryInteract()
{
    ...
    if (Physics.Raycast(transform.position + Vector3.up, transform.forward, out hit, interactDistance, interactableLayer))
    {
        ...
        // 2. Revisa si es un pickup de arma
        ...
        // 3. Revisa si es un pickup de radio
        RadioPickup radio = hit.collider.GetComponent<RadioPickup>();
        if (radio != null)
        {
            radio.Pickup(gameObject);
            return;
        }
    }
}
